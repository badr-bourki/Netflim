...
export default function MoviesPage() {
  ...
  const query = useInfiniteQuery({
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam }) => getMovies({ page: pageParam as number, genre: genre || undefined }),
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/70">Browse by genre with cached, paged results.</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6">
        {query.isLoading
          ...
          : items.map((m) => (
              <MovieCard key={`${m.id}`} item={m} mediaType={'movie' as MediaType} />
            ))}
      </div>
      ...
      {query.isError ? <div className="mt-6 text-sm text-netflim-red">Failed to load movies.</div> : null}
    </div>
  )
}
